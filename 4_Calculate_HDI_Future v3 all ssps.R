...
"
Scenario.key <- list('SSP1' = data.frame('Country_1' = c(0.66, 0.66, 0.66), 
                                         'Country_2' = c(0.33, 0.33, 0.33), 
                                         'Country_3' = c(0.1, 0.1, 0.1)), 
                     'SSP2' = data.frame('Country_1' = c(1, 1, 1), 
                                         'Country_2' = c(1, 1, 1), 
                                         'Country_3' = c(1, 1, 1)), 
                     'SSP3' = data.frame('Country_1' = c(1.33, 1.3, 1.33), 
                                         'Country_2' = c(1.1, 1.1, 1.1), 
                                         'Country_3' = c(1, 1, 1)), 
                     'SSP4' = data.frame('Country_1' = c(1.66, 1.66, 1.66), 
                                         'Country_2' = c(1.33, 1.33, 1.33), 
                                         'Country_3' = c(1.2, 1.2, 1.2)), 
                     'SSP5' = data.frame('Country_1' = c(1, 1, 1), 
                                         'Country_2' = c(1, 1, 1), 
                                         'Country_3' = c(1, 1, 1)))

Scenario.key <- lapply(Scenario.key, function(x) {
  
  row.names(x) <- c('Region_1', 'Region_2', 'Region_3')
...
test$Health_prediction <- test$Health_prediction * test$SSP.Adjustment

### multiply scenario weights by existing downscaling factor

test <- test %>% 
  split(test$GDLCODE) %>%
  lapply(function(y) {
    
    ineq.factor <- Scenario.key[y$SCENARIO]
    
    for(i in 1:length(ineq.factor)) {
      
      ineq.factor[[i]] <- ineq.factor[[i]][y$Country_tier[i], y$Tier[i]]
      
    }
    
    ### multiply
    y$Nat_reg_ratio <- ifelse(y$Year > 2020, 
                              1 + ((y$Health_nat_reg_ratio - 1) * unlist(ineq.factor)), y$Health_nat_reg_ratio)
    
    ### set to 0 if not 2100
    #y$Nat_reg_ratio[y$Year != 2100 | y$Year != 2020] <- NA  
...
### interpolate between 2020 - 2100
test$Nat_reg_ratio[!test$Year %in% c(2020, 2095)] <- NA

test <- test[test$ISO_Code != 'SYR', ]
test <- test %>% split(test$GDLCODE) %>%
  
  lapply(function(x) {try({
    
    y    <- split(x, x$SCENARIO)
    ts.y <- lapply(y, function(z) {zoo(z$Nat_reg_ratio, order.by = z$Year)}) 
    ts.y <- lapply(ts.y, function(z) {zoo::na.spline.default(z)})
    
    for(i in 1:length(y)) {
      
      y[[i]]$Nat_reg_ratio <- as.numeric(ts.y[[i]])
...
test %>% 
  ...
  summarise(index = mean(Nat_reg_ratio)) %>%
...
test <- merge(x[x$Year >= 2020, c(1, 2, 4:(ncol(x)))], Edu_SSP_key[, c(1, 3, 7:9)], 
              by = c('GDLCODE', 'Year'))
...
test <- test %>% 
        split(test$GDLCODE) %>%
          lapply(function(y) {
            ...
            ineq.factor <- Scenario.key[substr(y$SCENARIO, 1, 4)]
            ...
            ### multiply
            y$Nat_reg_ratio <- ifelse(y$Year > 2020, 
                                1 + ((y$Nat_reg_ratio - 1) * unlist(ineq.factor)), y$Nat_reg_ratio)
...
### interpolate between 2020 - 2100
test$Nat_reg_ratio[!test$Year %in% c(2020, 2100)] <- NA
